...
To skip the Bayesian modeling and just run the PCAs, set bayesian=FALSE
```{r mambo, include=FALSE}
result <- mambo(
  resp.label = '18s', 
  resp.counts = counts.18s, 
...
6) A list of the results for every replicate for just one locus
```{r}
...
```{r}
result$reps[[1]]$post.smry
```

...and a list of the actual posterior sample from the Bayesian linear model:
```{r}
result$reps[[1]]$post.list
```

## Plot results

# Import metadata files, which must include a "sample" column with sample IDs

```{r meta}
meta.16s <- read.csv(file = "Merged2018_16S_meta_filtered.csv")
meta.18s <- read.csv(file = "Merged2018_18S_meta_filtered.csv")
```

# Plot alpha diversity (the effective number of ASVs, Hill number = 1)

```{r diversity}
#16S
plotDiversity(result, '16s', type = "ellipse", pc.x = 1, 
              pc.y = 2, sample.df = meta.16s,
              ellipse.fill = 'Depth_group_m', facet.by = 'Cruise)
```

```{r diversity}
#18S
plotDiversity(result, '18s', type = "ellipse", pc.x = 1, 
              pc.y = 2, sample.df = meta.18s,
              ellipse.fill = 'Depth_group_m', facet.by = 'Cruise)
```
...
```{r PCA}
# default type = "ellipse", can change with type = "density"
#16S
plotPCs(result, '16s', pc.x = 1, pc.y = 2, 
        sample.df = meta.16s, ellipse.fill = 'Depth_group_m', 
        facet.by = 'Cruise')
```

```{r PCA}
#18S
plotPCs(result, '18s', pc.x = 1, pc.y = 2, 
        sample.df = meta.18s, ellipse.fill = 'Depth_group_m', 
        facet.by = 'Cruise')
```
...
# Summarize the posterior of predictive switches for a given number of 
# response and predictor PCs (here the first four of each)
```{r switch}
switchSummary(result, min.p = 0.9, resp.pcs = 1:4, pred.pcs = 1:4)
```
...
```{r taxa}
taxa.16s <- read.csv(file = "Merged2018_16S_taxa_filtered.csv")
taxa.18s <- read.csv(file = "Merged2018_18S_taxa_filtered.csv")
```
